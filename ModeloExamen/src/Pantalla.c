...
int pan_imprimirPantalla(Pantalla* pPantalla)
{
	...
	{
		retorno=0;
		printf("\nID:%d - tipo:%d - Nombre:%s - Direccion:%s - Precio:%.2f - isEmpty:%d",
				pPantalla->id,
				pPantalla->tipo,
				pPantalla->nombre,
				pPantalla->direccion,
 ...
 * \param array Array de Pantalla a ser actualizado
 * \param limite Limite del array de Pantalla
 * \param id Identificador a ser asignado al Pantalla
 * \return Retorna 0 (EXITO) y -1 (ERROR)
 *
 */
int pan_altaArrayPantalla(Pantalla* array,int limite)
{
	...
	if(array != NULL && limite > 0)
	{
		...
		if(auxIndice>=0)
		{
			if( !utn_getNumero(&bufferPantalla.tipo,"Ingrese el tipo de pantalla[1-2]: ","ERROR",MIN_TIPO,MAX_TIPO,QTY_REINT) &&
				!utn_getNombre(bufferPantalla.nombre,LENGTH_NOMBRE,"Ingrese Nombre Pantalla: ","ERROR, nombre invalido.\n",QTY_REINT) &&
				!utn_getTexto(bufferPantalla.direccion,LENGTH_DIRECCION,"Ingrese la Direccion: ","ERROR\n",QTY_REINT) &&
				!utn_getNumeroFlotante(&bufferPantalla.precio, "Ingrese el precio: ", "ERROR\n",MIN_PRECIO,MAX_PRECIO,QTY_REINT))
			{
...
int pan_modificarArrayPantalla(Pantalla* array,int limite)
{
	...
	if(array != NULL && limite > 0)
	{
		...
		utn_getNumero(&auxID,"\nIngresa el ID de la pantalla a modificar: ","ERROR",MIN_ID,MAX_ID,QTY_REINT);
		auxIndice = pan_buscarIdPantalla(array, limite,auxID);
		if(auxIndice>=0)
		{
			if(	!utn_getNumero(&bufferPantalla.tipo,"Ingrese el nuevo tipo de pantalla[1-2]: ","ERROR\n",MIN_TIPO,MAX_TIPO,QTY_REINT) &&
				!utn_getNombre(bufferPantalla.nombre,LENGTH_NOMBRE,"Ingrese nuevo Nombre Pantalla: ","ERROR, nombre invalido.\n",QTY_REINT) &&
				!utn_getTexto(bufferPantalla.direccion,LENGTH_DIRECCION,"Ingrese nueva Direccion: ","ERROR\n",QTY_REINT) &&
				!utn_getNumeroFlotante(&bufferPantalla.precio, "Ingrese el nuevo precio: ", "ERROR",MIN_PRECIO,MAX_PRECIO,QTY_REINT))
			{
